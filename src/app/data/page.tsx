...
function getValidatedSort(
  config: TableConfig,
  columnParam: string | undefined,
) {
  if (!columnParam) return undefined;
  return config.columns.some((column) => column.key === columnParam)
    ? columnParam
    : undefined;
}
...
export const metadata = {
  ...
  description:
    "Browse Medicare Advantage data tables, run AI-assisted analysis, and generate charts.",
};
...
export default async function DataPage({ searchParams }: PageProps) {
  ...
  const limit = Number.isFinite(limitParam)
    ? Math.min(
        Math.max(limitParam ?? DEFAULT_DISPLAY_ROWS, 10),
        MAX_DISPLAY_ROWS,
      )
    : DEFAULT_DISPLAY_ROWS;

  const supportsYear = config.columns.some((column) => column.key === "year");
  const supportsContract = config.columns.some(
    (column) => column.key === "contract_id",
  );

  const yearParamRaw =
    typeof params.year === "string" ? params.year.trim() : undefined;
  const yearParam = yearParamRaw
    ? Number.parseInt(yearParamRaw, 10)
    : undefined;
  const yearFilter =
    supportsYear && yearParam !== undefined && Number.isFinite(yearParam)
      ? yearParam
      : undefined;

  const contractParamRaw =
    typeof params.contract === "string" ? params.contract.trim() : undefined;
  const contractFilter =
    supportsContract && contractParamRaw ? contractParamRaw : undefined;
  ...
  const [tableData, filterOptions] = await Promise.all([
    ...
    supportsYear || supportsContract
      ? fetchFilterOptions(config, tableName)
      : Promise.resolve(emptyFilterOptions),
  ]);

  const { rows, count } = tableData;
  const { years: availableYears, contractIds: availableContracts } =
    filterOptions;
  ...
  if (supportsYear && yearFilter !== undefined)
    baseParams.set("year", String(yearFilter));
  if (supportsContract && contractFilter)
    baseParams.set("contract", contractFilter);

  const activeSort = sortColumn;
  const hasMoreRows = count > rows.length;
  const hasActiveFilters =
    searchTerm || yearFilter !== undefined || contractFilter;
  ...
  return (
    <AppShell
      title="Medicare Advantage Explorer"
      subtitle="Browse Medicare Advantage data tables and export filtered views."
      mainClassName="gap-6"
    >
        ...
        <div className="fep-card">
          ...
          <div className="flex flex-col gap-4 px-8 py-6">
              ...
              <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                ...
                <div className="flex flex-wrap items-center gap-3 text-xs text-muted-foreground">
                  {supportsYear && availableYears.length > 0 ? (
                    <label className="flex items-center gap-2">
                      ...
                      <select
                        ...
                        name="year"
                        defaultValue={
                          yearFilter !== undefined ? String(yearFilter) : ""
                        }
                      >
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {hasActiveFilters && (
                      <ClearFiltersButton tableName={tableName} />
                    )}
                    <ExportCsvButton
                      config={config}
            ...
            <div className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-border bg-muted px-4 py-3 text-xs text-muted-foreground">
              <div>
                Showing <span className="text-foreground">{rows.length}</span>{" "}
                of <span className="text-foreground">{count}</span> rows.
                {hasMoreRows
                  ? " Narrow filters or increase row limit to see more."
                  : null}
              </div>
              <div>
                Columns: {config.columns.length}. Searchable:{" "}
                {config.searchableColumns.length} fields.
              </div>
            </div>
          </div>
        </div>
