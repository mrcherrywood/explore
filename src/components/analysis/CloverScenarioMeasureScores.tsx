...
const SCENARIO_IDS: CloverComputedScenarioId[] = [
  "officialRecalc",
  "s29Removal",
  "model1",
  "model2",
...
function weightedStars(
  measures: Array<{ starValue: number; weight: number }>,
): number | null {
  let weightedSum = 0;
  let totalWeight = 0;
...
export function CloverScenarioMeasureScores({ contract, scenarios }: Props) {
  const [activeScenarioId, setActiveScenarioId] =
    useState<CloverComputedScenarioId>("officialRecalc");
  ...
  const activeScenario =
    scenarioOptions.find((scenario) => scenario.id === activeScenarioId) ??
    scenarioOptions[0];
  const measureScores = contract.scenarioMeasureScores ?? {};
  const allMeasures = activeScenario
    ? (measureScores[activeScenario.id] ?? [])
    : [];
  const removedMeasures = useMemo(
    () => allMeasures.filter((measure) => measure.removed),
    [allMeasures],
  );
  ...
  const measures = useMemo(
    () =>
      measureFilter === "all"
        ? allMeasures
        : measureFilter === "removed"
          ? removedMeasures
          : keptMeasures,
    [allMeasures, removedMeasures, keptMeasures, measureFilter],
  );
  const detail = activeScenario
    ? (contract.scenarioDetails[activeScenario.id] ?? null)
    : null;
    ...
    <section className="rounded-2xl border border-border bg-card">
      <div className="flex flex-col gap-3 border-b border-border p-4 md:flex-row md:items-center md:justify-between">
        <div>
          <h3 className="text-sm font-semibold text-foreground">
            Scenario Measure Scores
          </h3>
          <p className="text-xs text-muted-foreground">
            {contract.contractId} - measures removed from and kept in each
            ...
            {MEASURE_FILTERS.map((filter) => (
              ...
              >
                ...
                <span className="font-mono">
                  {formatStarScore(scoreByFilter[filter.id])}
                </span>
              </button>
            ))}
          </div>
        ...
        {activeScenario ? (
          <>
            ...
            <span className="text-rose-500">
              ...
              <span className="font-mono">
                {formatStarScore(scoreByFilter.removed)}
              </span>
              )
            </span>
            <span className="mx-2">/</span>
            <span className="text-emerald-500">
              {keptCount} kept (
              <span className="font-mono">
                {formatStarScore(scoreByFilter.kept)}
              </span>
              )
            </span>
            <span className="mx-2">-</span>
            <span>
              {allMeasures.length} total (
              <span className="font-mono">
                {formatStarScore(scoreByFilter.all)}
              </span>
              )
            </span>
          </>
      ...
      {detail && detail.score !== null ? (
        <div className="flex flex-wrap items-center gap-x-2 gap-y-1 border-b border-border bg-card px-4 py-3 text-xs text-muted-foreground">
          ...
          <span title="Reward factor (+0.1 to +0.4) from the mean/variance percentile thresholds.">
            Reward factor{" "}
            <span className="font-mono text-foreground">
              {formatSigned(detail.rewardFactor)}
            </span>
          </span>
          <span>+</span>
          <span title="Categorical Adjustment Index applied after the reward factor.">
            CAI{" "}
            <span className="font-mono text-foreground">
              {formatSigned(detail.caiValue)}
            </span>
          </span>
          <span>=</span>
          <span
            className="font-semibold text-foreground"
            title="Calculated scenario score before hold-harmless rounding."
          >
            <span className="font-mono">{formatNumber(detail.score)}</span>
            <span className="ml-1 font-mono text-muted-foreground">
              ({roundToHalf(detail.score).toFixed(1)}★)
            </span>
          </span>
          {detail.holdHarmlessApplied ? (
            <span
              className="ml-1 rounded-full bg-amber-500/10 px-2 py-0.5 text-[10px] font-medium text-amber-500"
              title="The base mean uses the without-QI calculation because dropping the QI measure lifts this contract to 4+ stars."
            >
              QI hold-harmless
            </span>
          ) : null}
          ...
          <thead>
            <tr className="border-b border-border bg-muted/30 text-xs text-muted-foreground">
              <th
                ...
                title="Whether the measure is removed from or kept in this scenario's calculation"
              >
                Status
              </th>
              <th className="px-4 py-3 text-left" title="CMS measure code">
                Code
              </th>
              <th className="px-4 py-3 text-left" title="CMS measure name">
                Measure
              </th>
              <th className="px-4 py-3 text-left" title="Part C or Part D">
                Part
              </th>
              <th
                className="px-4 py-3 text-right"
                title="Underlying measure score/rate from CMS measure data"
              >
                Score
              </th>
              <th
                className="px-4 py-3 text-right"
                title="Measure-level star rating used in the scenario calculation"
              >
                Stars
              </th>
              <th
                className="px-4 py-3 text-right"
                title="CMS weight used in the weighted score calculation"
              >
                Weight
              </th>
            </tr>
            ...
            {measures.map((measure, index) => (
              ...
              >
                ...
                <td className="px-4 py-3 text-right font-mono text-xs">
                  {formatMeasureValue(measure.measureValue)}
                </td>
                <td className="px-4 py-3 text-right font-mono text-xs">
                  {formatStar(measure.starValue)}
                </td>
                <td className="px-4 py-3 text-right font-mono text-xs">
                  {measure.weight}
                </td>
      ...
      {measures.length === 0 ? (
        <div className="p-6 text-center text-sm text-muted-foreground">
          No {measureFilter === "all" ? "" : `${measureFilter} `}measures found
          for this scenario.
        </div>
      ) : null}
    </section>
