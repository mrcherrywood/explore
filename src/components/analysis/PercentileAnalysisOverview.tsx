...
const scriptCards = [
  {
    ...
    description:
      "Calculates a percentile rank for every H and R contract on every measure in each CMS measure-data file.",
    details: [
  ...
  },
  {
    ...
    description:
      "Translates CMS cut points into the percentile they actually represent inside the observed H-contract distribution.",
    details: [
    ...
    <div className="flex flex-col gap-6">
        ...
        <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
          <div className="max-w-3xl">
            ...
            <h2 className="mt-2 text-3xl font-semibold text-foreground">
              Percentile analysis for CMS Stars measure data
            </h2>
            <p className="mt-4 text-sm leading-6 text-muted-foreground">
              This page summarizes the workflow in
              `scripts/percentile-analysis`: one script scores every contract
              against the full market distribution, and the other shows where
              each CMS cut point sits inside that distribution.
            </p>
          </div>
          <div className="grid gap-3 text-sm text-muted-foreground sm:grid-cols-3">
            <div className="rounded-2xl border border-border bg-background/60 p-4">
              ...
              <p className="mt-2 text-xs leading-5">
                Contract-level ranking plus cut point percentile translation.
              </p>
            </div>
            <div className="rounded-2xl border border-border bg-background/60 p-4">
              <div className="flex items-center gap-2 text-foreground">
              ...
              <p className="mt-2 text-xs leading-5">
                Excel-style `PERCENTRANK.INC` and scipy `percentileofscore`.
              </p>
            </div>
            <div className="rounded-2xl border border-border bg-background/60 p-4">
              <div className="flex items-center gap-2 text-foreground">
              ...
              <p className="mt-2 text-xs leading-5">
                Shared config maps each year to its source file and header
                layout.
              </p>
            </div>
          </div>
          ...
          >
            ...
            <ul className="mt-5 space-y-3 text-sm text-muted-foreground">
              {card.details.map((detail) => (
                <li
                  key={detail}
                  className="rounded-2xl border border-border bg-background/50 px-4 py-3 leading-6"
                >
                  {detail}
                </li>
              ))}
      ...
      <section className="grid gap-6 xl:grid-cols-[1.2fr_0.8fr]">
        <article className="rounded-3xl border border-border bg-card p-8">
          ...
          <h3 className="mt-2 text-xl font-semibold text-foreground">
            Choose the rank convention explicitly
          </h3>
        ...
        <article className="rounded-3xl border border-border bg-card p-8">
          ...
          <ul className="mt-5 space-y-2 text-sm text-muted-foreground">
            {expectedFiles.map((file) => (
              <li
                key={file}
                className="rounded-2xl border border-border bg-background/50 px-4 py-3 font-mono text-xs leading-5"
              >
                {file}
              </li>
            ))}
      ...
      <section className="grid gap-6 xl:grid-cols-2">
        <article className="rounded-3xl border border-border bg-card p-8">
          ...
          <p className="mt-4 text-sm leading-6 text-muted-foreground">
            Produces either Excel or JSON output with one record per contract
            and score/percentile pairs for each measure.
          </p>
        </article>
        ...
        <article className="rounded-3xl border border-border bg-card p-8">
          ...
          <p className="mt-4 text-sm leading-6 text-muted-foreground">
            Adds percentile equivalents plus distribution notes so the cut
            points can be interpreted against the real plan spread.
          </p>
        </article>
      </section>
      ...
      <section className="rounded-3xl border border-border bg-card p-8">
        ...
        <h3 className="mt-2 text-xl font-semibold text-foreground">
          Important behavior baked into shared config
        </h3>
        <div className="mt-6 grid gap-4 lg:grid-cols-2">
          {designNotes.map((note) => (
            <div
              key={note}
              className="rounded-2xl border border-border bg-background/50 px-5 py-4 text-sm leading-6 text-muted-foreground"
            >
              {note}
            </div>
          ))}
