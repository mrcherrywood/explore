    ...
    async function load() {
      ...
      try {
        ...
        if (
          ...
          throw new Error(
            payload.error || "Failed to load measure percentile analysis",
          );
        }
        ...
        setError(
          err instanceof Error
            ? err.message
            : "Failed to load measure percentile analysis",
        );
        setLoadState("error");
      }
    }
      ...
      <div className="flex flex-col gap-5 xl:flex-row xl:items-end xl:justify-between">
        <div className="max-w-3xl">
          ...
          <h2 className="mt-2 text-2xl font-semibold text-foreground">
            Measure star percentile equivalents
          </h2>
          <p className="mt-3 text-sm leading-6 text-muted-foreground">
            Pick a measure and star rating to see the percentile equivalent of
            that CMS cut point historically and for 2026.
          </p>
        </div>
      </div>
        ...
        <div className="mt-6 rounded-2xl border border-border bg-muted/20 p-6">
          <div className="flex items-center gap-3 text-red-400">
            ...
            <p className="font-medium">
              The measure percentile analysis could not be loaded.
            </p>
          </div>
        ...
        <div className="mt-6 space-y-6">
          <div className="rounded-2xl border border-border bg-muted/20 p-4">
            <p className="text-sm text-foreground">
              Measure:{" "}
              <span className="font-semibold">{data.selectedMeasure}</span>
              {" • "}
          ...
          <section className="rounded-3xl border border-border bg-card p-5">
            <h3 className="text-sm font-semibold text-foreground">
              Historical 2024-2026
            </h3>
            <p className="mt-1 text-xs text-muted-foreground">
              Sample-size-weighted summary across available years.
            </p>
            <div className="mt-4 grid gap-3 sm:grid-cols-3">
              <div className="rounded-2xl border border-border bg-muted/20 p-4">
                ...
                <div className="mt-2 text-2xl font-semibold text-foreground">
                  {typeof data.historicalSummary?.weightedAveragePercentile ===
                  "number"
                    ? formatPercent(
                        data.historicalSummary.weightedAveragePercentile,
                      )
                    : "—"}
                </div>
              </div>
              ...
              <div className="rounded-2xl border border-border bg-muted/20 p-4">
                ...
                <div className="mt-2 text-lg font-semibold text-foreground">
                  {data.historicalSummary?.totalSampleSize ?? 0}
                </div>
              </div>
            </div>
          </section>
          ...
          <section className="overflow-hidden rounded-3xl border border-border bg-card">
            <div className="border-b border-border px-5 py-4">
              <h3 className="text-sm font-semibold text-foreground">
                Year-by-year percentile equivalents
              </h3>
              <p className="mt-1 text-xs text-muted-foreground">
                Percentile associated with the selected star cut point in each
                year.
              </p>
            </div>
                ...
                <thead className="bg-muted/30 text-muted-foreground">
                  <tr>
                    <th
                      ...
                      title="CMS Star Ratings measurement year"
                    >
                      Year
                    </th>
                    <th
                      className="px-4 py-3 text-left font-medium"
                      title="CMS cut point score required for the selected star level in that year"
                    >
                      Cut point score
                    </th>
                    <th
                      className="px-4 py-3 text-left font-medium"
                      title="Percentile equivalent of the cut point score within the contract population (using the selected method)"
                    >
                      Associated percentile
                    </th>
                    <th
                      className="px-4 py-3 text-left font-medium"
                      title="Number of contracts with valid scores used in the percentile calculation"
                    >
                      Sample size
                    </th>
                  </tr>
                ...
                <tbody>
                  {data.yearlyResults.map((result) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-3 text-foreground">
                        {result.cutPointScore}
                      </td>
                      <td className="px-4 py-3 font-medium text-foreground">
                        {typeof result.percentileEquivalent === "number"
                          ? formatPercent(result.percentileEquivalent)
                          : "—"}
                      </td>
                      <td className="px-4 py-3 text-foreground">
                        {result.sampleSize}
