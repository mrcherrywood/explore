...
export function RewardFactorProjection() {
  ...
  const [mode, setMode] = useState<"full_market" | "client_only">(
    "full_market",
  );
  ...
  const filteredContracts = useMemo(() => {
    ...
    if (search)
      list = list.filter((c) => c.contractId.includes(search.toUpperCase()));
  ...
  const rFactorDistribution = useMemo(() => {
    ...
    const contracts = clientOnly
      ? activeResult.contractResults.filter((c) => c.isClient)
      : activeResult.contractResults;
        ...
        <div className="flex flex-wrap items-end gap-4">
          <div className="flex-1">
            ...
            <p className="mt-1 text-[0.65rem] text-muted-foreground">
              Columns: contract_id, measure_code, projected_score,
              projected_star (score or star required)
            </p>
          </div>
          <div>
            <label
          ...
          <div>
            ...
            <select
              ...
              value={mode}
              onChange={(e) =>
                setMode(e.target.value as "full_market" | "client_only")
              }
              className="rounded-lg border border-border bg-background px-3 py-2 text-sm"
            >
      ...
      {data && activeResult && (
        <>
          ...
          <div className="grid grid-cols-4 gap-4">
            <SummaryCard
              label="Population"
              value={activeResult.populationSize}
              subtitle={`${activeResult.clientContractCount} client contracts`}
            />
            <SummaryCard
              label="Projected Measures"
              value={data.projectedMeasureCount}
              subtitle={`${data.clientContracts} contracts`}
            />
            <SummaryCard
              label="Contracts with r-Factor > 0"
              value={
                activeResult.contractResults.filter((c) => c.rFactor > 0).length
              }
            ...
            <SummaryCard
              ...
              value={
                activeResult.contractResults.filter(
                  (c) =>
                    c.qiSensitiveRFactorRange[0] !==
                    c.qiSensitiveRFactorRange[1],
                ).length
              }
          ...
          {rFactorDistribution && (
            <DistributionPanel distribution={rFactorDistribution} />
          )}
          ...
          <div className="rounded-xl border border-border bg-card">
            <div className="flex items-center justify-between border-b border-border px-5 py-3">
              ...
              <div className="flex items-center gap-3">
                ...
                <ExportCsvButton
                  fileName={`rfactor-projection-${year}`}
                  getData={getCsvData}
                />
              </div>
            </div>
            <div className="max-h-[500px] overflow-auto">
                ...
                <thead className="sticky top-0 bg-muted/50">
                  <tr>
                    ...
                    <th
                      ...
                      onClick={() => handleSort("weightedMean")}
                    >
                      Mean
                      <SortIcon col="weightedMean" />
                    </th>
                    <th
                      className="cursor-pointer px-3 py-2 text-right font-medium"
                      onClick={() => handleSort("weightedVariance")}
                    >
                      Variance
                      <SortIcon col="weightedVariance" />
                    </th>
                    ...
                    <th
                      className="cursor-pointer px-3 py-2 text-right font-medium"
                      onClick={() => handleSort("measureCount")}
                    >
                      Measures
                      <SortIcon col="measureCount" />
                    </th>
                  </tr>
                </thead>
                <tbody>
                    ...
                    return (
                      ...
                      >
                        <td className="px-3 py-1.5 font-mono">
                          {c.contractId}
                          {c.isClient && (
                            <span className="ml-1.5 rounded bg-primary/10 px-1 py-0.5 text-[0.6rem] text-primary">
                              client
                            </span>
                          )}
                        </td>
                        ...
                        <td className="px-3 py-1.5 text-center">
                          <CategoryBadge category={c.meanCategory} />
                        </td>
                        <td className="px-3 py-1.5 text-center">
                          <CategoryBadge category={c.varianceCategory} />
                        </td>
                        <td className="px-3 py-1.5 text-right font-mono font-semibold">
                          {c.rFactor.toFixed(1)}
                        </td>
                        <td
                          className={`px-3 py-1.5 text-center font-mono ${qiSensitive ? "text-amber-500" : "text-muted-foreground"}`}
                        >
                          {qiSensitive
                            ? `${c.qiSensitiveRFactorRange[0]}-${c.qiSensitiveRFactorRange[1]}`
                            : "stable"}
                        </td>
              ...
              {filteredContracts.length > 200 && (
                <p className="p-3 text-center text-xs text-muted-foreground">
                  Showing 200 of {filteredContracts.length} contracts
                </p>
              )}
            </div>
          </div>
...
function CategoryBadge({ category }: { category: string }) {
  ...
  return (
    <span
      className={`inline-block rounded px-1.5 py-0.5 text-[0.6rem] font-medium ${colors[category] ?? "bg-muted text-muted-foreground"}`}
    >
    ...
    <div className="rounded-xl border border-border bg-card p-5">
      <div className="mb-3 flex items-center gap-2">
        <h3 className="text-sm font-semibold">
          Modeled Thresholds — {label} (Without QI)
        </h3>
        <div className="group relative">
          ...
          <div className="absolute bottom-full left-0 mb-1 hidden w-64 rounded-lg border border-border bg-popover p-2 text-xs text-muted-foreground shadow-lg group-hover:block">
            Primary thresholds computed without improvement measures (C30/D04).
            The &quot;Official&quot; column shows CMS published thresholds for
            the &quot;without improvement, with new measures&quot; scenario.
          </div>
        </div>
      </div>
      <table className="w-full text-xs">
        ...
        <tbody>
          {[
            ...
            <tr key={key} className="border-t border-border/50">
              ...
              {official && (
                <td className="px-3 py-1.5 text-right font-mono">
                  {fmt(official.official[key], 6)}
                </td>
              )}
              {official && (
                <td
                  className={`px-3 py-1.5 text-right font-mono ${Math.abs(official.differences[key]) < 0.01 ? "text-emerald-500" : Math.abs(official.differences[key]) < 0.05 ? "text-amber-500" : "text-red-500"}`}
                >
                  {official.differences[key] >= 0 ? "+" : ""}
                  {fmt(official.differences[key], 6)}
                </td>
              )}
              {official && (
  ...
  return (
    <div className="rounded-xl border border-border bg-card p-5">
      <h3 className="mb-3 text-sm font-semibold">
        QI Sensitivity Band (With Improvement Measures)
      </h3>
      <p className="mb-3 text-xs text-muted-foreground">
        Shows how thresholds shift when QI measures (C30/D04, weight 5) are
        added at each star level. The true &quot;with improvement&quot;
        thresholds fall within this band.
      </p>
      <table className="w-full text-xs">
        <thead>
          <tr className="border-b border-border">
        ...
        <tbody>
          {qiSensitivity.map((qi) => (
            <tr key={qi.qiStar} className="border-t border-border/50">
              ...
              <td className="px-3 py-1.5 text-right font-mono">
                {fmt(qi.thresholds.mean65th, 6)}
              </td>
              <td className="px-3 py-1.5 text-right font-mono">
                {fmt(qi.thresholds.mean85th, 6)}
              </td>
              <td className="px-3 py-1.5 text-right font-mono">
                {fmt(qi.thresholds.variance30th, 6)}
              </td>
              <td className="px-3 py-1.5 text-right font-mono">
                {fmt(qi.thresholds.variance70th, 6)}
              </td>
            </tr>
          ))}
          <tr className="border-t-2 border-border bg-muted/30 font-semibold">
            ...
            <td className="px-3 py-1.5 text-right font-mono">
              {fmt(qiBand.min.mean65th, 4)}–{fmt(qiBand.max.mean65th, 4)}
            </td>
            <td className="px-3 py-1.5 text-right font-mono">
              {fmt(qiBand.min.mean85th, 4)}–{fmt(qiBand.max.mean85th, 4)}
            </td>
            <td className="px-3 py-1.5 text-right font-mono">
              {fmt(qiBand.min.variance30th, 4)}–
              {fmt(qiBand.max.variance30th, 4)}
            </td>
            <td className="px-3 py-1.5 text-right font-mono">
              {fmt(qiBand.min.variance70th, 4)}–
              {fmt(qiBand.max.variance70th, 4)}
            </td>
          </tr>
        </tbody>
      </table>
    ...
    <div className="rounded-xl border border-border bg-card p-5">
      ...
      <div className="flex items-end gap-3">
        {[0, 0.1, 0.2, 0.3, 0.4].map((rf) => {
          ...
          const height =
            maxCount > 0 ? Math.max(4, (count / maxCount) * 120) : 4;
          return (
