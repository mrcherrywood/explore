...
export function QualityImprovementAnalysis() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      setError(null);
      try {
        const response = await fetch("/api/analysis/quality-improvement", {
          signal: controller.signal,
        });
        if (!response.ok) {
          const payload = await response.json().catch(() => ({}));
          throw new Error(
            payload.error || "Failed to load quality improvement analysis",
          );
        }
        ...
        setError(
          err instanceof Error
            ? err.message
            : "Failed to load quality improvement analysis",
        );
      } finally {
        setIsLoading(false);
      }
  ...
  const totalSamplePoints = useMemo(() => {
    if (!data) return 0;
    return data.measures.reduce(
      (sum, measure) => sum + measure.samplePoints,
      0,
    );
  }, [data]);
        ...
        <div className="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
          <div className="max-w-2xl space-y-3">
            <p className="text-xs uppercase tracking-[0.6em] text-slate-400">
              Quality Improvement Watch
            </p>
            <h2 className="text-3xl font-semibold">
              Are high-scoring contracts able to stay ahead?
            </h2>
            <p className="text-sm text-slate-300">
              We tracked Part C (Health Plan) and Part D (Drug Plan) Quality
              Improvement star ratings across recent CMS Star Rating years to
              quantify how often contracts sustain elite performance. The data
              tells us whether earning a 4-5★ score simply raises the bar for
              the following year.
            </p>
          </div>
        ...
        <>
          <section className="rounded-3xl border border-border bg-card p-6">
            <div className="mb-4 flex items-center gap-3">
              <ActivitySquare className="h-5 w-5 text-[var(--fep-accent)]" />
              <div>
                <h3 className="text-base font-semibold text-foreground">
                  High-performer watchlist
                </h3>
                <p className="text-xs text-muted-foreground">
                  Key signals from the latest contiguous year comparisons
                </p>
              </div>
            </div>
              ...
              <div className="rounded-3xl border border-border bg-card p-6">
                <div className="mb-4 flex items-center justify-between">
                  <div>
                    ...
                    <h3 className="text-base font-semibold text-foreground">
                      Average quality improvement stars
                    </h3>
                  </div>
                  <LineChart className="h-5 w-5 text-[var(--fep-accent)]" />
                </div>
              ...
              <div className="rounded-3xl border border-border bg-card p-6">
                <div className="mb-4 flex items-center justify-between">
                  <div>
                    ...
                    <h3 className="text-base font-semibold text-foreground">
                      What happens after a 4-5★ score
                    </h3>
                  </div>
            ...
            <div className="grid gap-4 lg:grid-cols-2">
              {data.measures.map((measure) => {
                const latestTransition =
                  measure.transitions[measure.transitions.length - 1];
                    ...
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <h4 className="text-base font-semibold text-foreground">
                          {latestTransition.fromYear} →{" "}
                          {latestTransition.toYear}
                        </h4>
                      </div>
                      ...
                      <div className="mt-2 space-y-2 text-sm text-foreground">
                        {latestTransition.stayRates
                          ...
                          .map((row) => (
                            <div
                              key={row.rating}
                              className="grid grid-cols-3 gap-2 rounded-xl border border-border/50 px-3 py-2 text-sm"
                            >
                              <span className="font-medium">{row.rating}★</span>
                              <span className="text-center font-semibold text-[var(--fep-accent)]">
                                {row.stayRate !== null
                                  ? `${row.stayRate}%`
                                  : "—"}
                              </span>
                    ...
                    >
                      <div className="flex items-center justify-between">
                        ...
                        <div className="text-right">
                          ...
                          <p className="text-xs text-muted-foreground">
                            {latest?.highShare ?? "—"}% at 4-5★ ·{" "}
                            {latest?.perfectShare ?? "—"}% at 5★
                          </p>
                        </div>
                      </div>
                    </div>
