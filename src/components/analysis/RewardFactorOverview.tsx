...
export function RewardFactorOverview() {
  ...
  const getCsvData = useCallback(() => {
    ...
    const rows = filteredContracts.map((c) => [
      ...
      c.weightedMean.toFixed(6),
      c.weightedVariance.toFixed(6),
  ...
  const pctWith =
    data.contracts.length > 0
      ? ((rfWithReward / data.contracts.length) * 100).toFixed(1)
      : "0";
  const pctWithout =
    data.contracts.length > 0
      ? ((rfWithout / data.contracts.length) * 100).toFixed(1)
      : "0";
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <p className="text-xs text-muted-foreground">
            {data.ratingLabel} · {data.populationSize} contracts ·{" "}
            {rfWithReward} with reward factor
          </p>
        </div>
        <div className="flex items-center gap-2">
          <label htmlFor="rf-year" className="text-xs text-muted-foreground">
            Year
          </label>
          <select
            id="rf-year"
        ...
        <div className="rounded-xl border border-amber-500/30 bg-amber-500/5 px-5 py-4">
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              <p className="text-sm font-medium text-[#9a7415]">
                Projected from {data.sourceYear} contract data
              </p>
              <p className="mt-1 text-xs text-muted-foreground">
                {data.removedMeasures.length} measures removed per announced CMS
                retirements. All contract scores are pulled forward from{" "}
                {data.sourceYear} with retired measures excluded.
              </p>
              <button
                type="button"
                onClick={() => setShowRemovedMeasures(!showRemovedMeasures)}
                className="mt-2 text-xs font-medium text-amber-600 hover:underline text-[#9a7415]"
              >
                {showRemovedMeasures ? "Hide" : "Show"} removed measures (
                {data.removedMeasures.length})
              </button>
              {showRemovedMeasures && (
      ...
      <div className="grid grid-cols-5 gap-4">
        <div className="rounded-xl border border-border bg-card p-4">
          ...
          <p className="mt-0.5 text-xs text-emerald-500">
            {pctWith}% received reward factor
          </p>
        ...
        {rewardBars.map((rf) => {
          ...
          const colors =
            rf <= 0.2 ? "border-blue-500/30" : "border-emerald-500/30";
          return (
            <div key={rf} className={`rounded-xl border bg-card p-4 ${colors}`}>
              <p className="text-xs text-muted-foreground">
                r-Factor {rf.toFixed(1)}
              </p>
      ...
      <div className="grid grid-cols-2 gap-4">
        <ThresholdCard
          set={data.thresholdsWithQI}
          isProjected={data.isProjected}
        />
        <ThresholdCard
          set={data.thresholdsWithoutQI}
          isProjected={data.isProjected}
        />
      </div>
      ...
      <div className="rounded-xl border border-border bg-card p-5">
        <div className="mb-3 flex items-center justify-between">
          ...
          <p className="text-xs text-muted-foreground">
            {rfWithReward} of {data.contracts.length} contracts ({pctWith}%)
            received reward factor
          </p>
        </div>
        <div className="flex items-end gap-3">
          {rewardBars.map((rf) => {
            ...
            const rfPct =
              rfWithReward > 0
                ? ((count / rfWithReward) * 100).toFixed(1)
                : "0";
            const height =
            ...
            return (
              <div key={rf} className="flex flex-1 flex-col items-center gap-1">
                ...
                <div
                  ...
                  style={{
                    height,
                    background: `hsl(${120 + rf * 200}, 60%, 50%)`,
                  }}
                />
      ...
      <div className="rounded-xl border border-border bg-card">
        <div className="flex items-center justify-between border-b border-border px-5 py-3">
          <div>
            ...
            {data.isProjected && (
              <p className="mt-0.5 text-[0.6rem] text-muted-foreground">
                Contracts qualifying for reward factor using {data.sourceYear}{" "}
                scores with retired measures removed
              </p>
            )}
          </div>
          <div className="flex items-center gap-3">
            ...
            <ExportCsvButton
              fileName={`rfactor-${year}-overall-mapd`}
              getData={getCsvData}
            />
          </div>
        </div>
        <div className="max-h-[500px] overflow-auto">
            ...
            <tbody>
              {filteredContracts.slice(0, 300).map((c) => (
                ...
                >
                  ...
                  <td
                    className="px-3 py-1.5 max-w-[200px] truncate text-muted-foreground"
                    title={c.parentOrganization ?? ""}
                  >
                    {c.parentOrganization ?? "—"}
                  </td>
                  <td className="px-3 py-1.5 text-right font-mono">
                    {c.weightedMean.toFixed(4)}
                  </td>
                  <td className="px-3 py-1.5 text-right font-mono">
                    {c.weightedVariance.toFixed(4)}
                  </td>
          ...
          {filteredContracts.length > 300 && (
            <p className="p-3 text-center text-xs text-muted-foreground">
              Showing 300 of {filteredContracts.length} contracts
            </p>
          )}
        </div>
      </div>
            ...
            <tr key={k} className="border-t border-border/50">
              ...
              {hasOfficial && set.percentDifferences && (
                <td className="px-2 py-1.5 text-right">
                  <AccuracyBadge pctDiff={set.percentDifferences[k]} />
                </td>
              )}
            </tr>
          ))}
