  ...
  if (isLoading) {
    return (
      <div className="rounded-2xl border border-border bg-card p-8 text-sm text-muted-foreground">
        Running roster accuracy simulation...
      </div>
    );
  }
    ...
    <section className="rounded-2xl border border-border bg-card p-6">
      <div className="mb-2 flex items-center gap-3">
        ...
        <div>
          <h3 className="text-base font-semibold text-foreground">
            Roster Size vs Accuracy
          </h3>
          <p className="text-xs text-muted-foreground">
            {displayName} · {data.years.join("–")} avg ·{" "}
            {data.curve[0]?.trials ?? 0} trials per size
          </p>
        </div>
      </div>
        ...
        <ResponsiveContainer width="100%" height="100%">
          ...
          >
            ...
            <Tooltip
              content={
                <CurveTooltip
                  clientSize={data.clientRosterSize}
                  marketSize={data.fullMarketAvgSize}
                />
              }
            />
            <Area
      ...
      <div className="mt-3 flex flex-wrap items-center gap-4 text-xs text-muted-foreground">
        <span className="flex items-center gap-1.5">
          <span className="inline-block h-2.5 w-2.5 rounded-full bg-[var(--fep-accent)]" />
      ...
      <p className="mt-3 text-xs text-muted-foreground">
        ...
        Each point runs {data.curve[0]?.trials ?? 0} random subsamples of that
        size from the full market, backtests each against official CMS cut
        points across {data.years.length} years, and averages the MAE. The
        shaded band shows the best and worst trial at each size.
      </p>
    </section>
  );
}
