...
function formatEnrollmentLabel(value: number | null): string {
  return value === null || value <= 0
    ...
    : `${formatEnrollment(value)} members`;
}
...
function getContractQbpChange(
  ...
  return (
    (scenarioEligible ? 1 : -1) *
    (contract.totalEnrollment ?? 0) *
    ESTIMATED_ANNUAL_QBP_PER_MEMBER
  );
}
  ...
  const visible = contracts.slice(0, 5).map((contract) => {
    const enrollment =
      contract.enrollment === null
        ? "enrollment unavailable"
        : `${contract.enrollment.toLocaleString()} members`;
...
export function CloverParentOrgChart({
  parentOrganization,
  contracts,
  chartScores,
  enrollmentSource,
  ...
  const parentContracts = useMemo(
    () =>
      contracts.filter(
        (contract) =>
          (contract.parentOrganization?.trim() || "Unknown") ===
          parentOrganization,
      ),
    [contracts, parentOrganization],
  );
  const enrollmentContracts = parentContracts.filter(
    (contract) => (contract.totalEnrollment ?? 0) > 0,
  );
  const totalEnrollment = enrollmentContracts.reduce(
    (sum, contract) => sum + (contract.totalEnrollment ?? 0),
    0,
  );
  ...
  const chartData: ChartDatum[] = chartScores
    .map((score) => {
      const value =
        weightMode === "equal"
          ? average(
              parentContracts.map((contract) => contract.scores[score.id]),
            )
          : weightedAverage(
      ...
      const baseMean =
        weightMode === "equal"
          ? average(
              parentContracts.map(
                (contract) =>
                  getScoreDetail(contract, score.id)?.baseMean ?? null,
              ),
            )
          : weightedAverage(
              parentContracts.map((contract) => ({
      ...
      const rewardFactor =
        weightMode === "equal"
          ? average(
              parentContracts.map(
                (contract) =>
                  getScoreDetail(contract, score.id)?.rewardFactor ?? null,
              ),
            )
          : weightedAverage(
              parentContracts.map((contract) => ({
      ...
      const caiValue =
        weightMode === "equal"
          ? average(
              parentContracts.map(
                (contract) =>
                  getScoreDetail(contract, score.id)?.caiValue ?? null,
              ),
            )
          : weightedAverage(
              parentContracts.map((contract) => ({
      ...
      const scoredContractCount = parentContracts.filter(
        (contract) => contract.scores[score.id] !== null,
      ).length;
      const rewardFactorContractCount = parentContracts.filter(
        (contract) =>
          (getScoreDetail(contract, score.id)?.rewardFactor ?? 0) > 0,
      ).length;
      ...
      const qbpChange =
        qbpChanges.length > 0
          ? qbpChanges.reduce((sum, change) => sum + change, 0)
          : null;
      const qbpChangedContracts = parentContracts
        ...
        .filter(
          (contract): contract is QbpMovementContract & { change: number } =>
            contract.change !== null && contract.change !== 0,
        )
  ...
  const qbpExplainer =
    model1Entry &&
    model2Entry &&
    model1Entry.qbpChange !== null &&
    model2Entry.qbpChange !== null
      ? {
          model1: model1Entry,
          model2: model2Entry,
  ...
  const official2026 =
    weightMode === "equal"
      ? average(
          parentContracts.map((contract) => contract.officialScores.stars2026),
        )
      : weightedAverage(
          parentContracts.map((contract) => ({
  ...
  const weightModeLabel =
    weightMode === "equal" ? "Equal Contracts" : "Enrollment Weighted";
      ...
      >
        <div className="export-chart-header mb-4 pr-28">
          ...
          <p className="mt-1 text-xs text-muted-foreground">
            {parentOrganization} - {parentContracts.length} contracts,{" "}
            {formatEnrollmentLabel(totalEnrollment)}
          </p>
          <div className="export-chart-header-meta mt-2 flex flex-wrap items-center gap-x-6 gap-y-2 text-xs text-muted-foreground">
            <span>
              ...
              <span className="font-mono text-foreground">
                {formatScore(official2026)}
              </span>
            </span>
            <span className="export-only">Weighting: {weightModeLabel}</span>
            <div
              data-export-hide
              className="flex items-center gap-2 rounded-xl border border-border bg-muted p-1"
            >
              <button
                type="button"
        ...
        {weightMode === "enrollment" && enrollmentContracts.length === 0 ? (
          <div className="rounded-xl border border-amber-500/30 bg-amber-500/5 p-4 text-sm text-[#9a7415]">
            No positive enrollment values were found for this parent
            organization
                    ...
                    return (
                      <div className="rounded-xl border border-border bg-card p-3 text-xs shadow-lg">
                        ...
                        <p className="mt-2 text-muted-foreground">
                          {weightMode === "equal"
                            ? "Equal Avg / Rounded Star"
                            : "Enrollment Weighted Avg / Rounded Star"}
                          :{" "}
                          <span className="font-mono text-foreground">
                        ...
                        {entry.rewardFactor !== null ? (
                          <p className="mt-1 text-muted-foreground">
                            RF Contracts:{" "}
                            <span className="font-mono text-foreground">
                              {entry.rewardFactorContractCount} of{" "}
                              {entry.scoredContractCount}
                            </span>
                          </p>
                        ) : null}
                        {entry.caiValue !== null ? (
                        ...
                        <p className="mt-1 text-muted-foreground">
                          Parent Enrollment:{" "}
                          <span className="font-mono text-foreground">
                            {formatEnrollmentLabel(entry.totalEnrollment)}
                          </span>
                        </p>
                        {entry.qbpChange !== null ? (
                          <p className="mt-1 text-muted-foreground">
                            Est. Parent QBP Swing:{" "}
                            <span className="font-mono text-foreground">
                        ...
                        {entry.qbpChange !== null &&
                        (entry.qbpGainCount > 0 || entry.qbpLossCount > 0) ? (
                          <p className="mt-1 text-muted-foreground">
                            QBP G/L:{" "}
                            <span className="font-mono text-foreground">
        ...
        <div className="export-reward-factor-strip mt-4 rounded-xl border border-border bg-muted/30 p-3 text-[11px] text-muted-foreground">
          {rewardFactorEntries.length > 0 ? (
            <div className="export-reward-factor-list flex flex-wrap gap-2">
              ...
              {rewardFactorEntries.map((entry) => (
                ...
                >
                  ...
                  <span>
                    ({entry.rewardFactorContractCount} of{" "}
                    {entry.scoredContractCount} contracts)
                  </span>
                </span>
              ))}
            </div>
            ...
            <span>
              No reward factor applied to the calculated bars for this parent
              view.
            </span>
          )}
        </div>
        ...
        {qbpExplainer ? (
          <div className="export-qbp-note mt-3 rounded-xl border border-[var(--fep-info-border)] bg-[var(--fep-info-bg)] p-3 text-[11px] text-muted-foreground">
            ...
            ({qbpExplainer.model1.qbpGainCount} gain /{" "}
            {qbpExplainer.model1.qbpLossCount} lose), while Model 2 has an
            estimated net QBP swing of{" "}
            <span
              className={
                qbpExplainer.model2Qbp >= 0
                  ? "font-mono text-emerald-400"
                  : "font-mono text-rose-400"
              }
            >
            ...
            ({qbpExplainer.model2.qbpGainCount} gain /{" "}
            {qbpExplainer.model2.qbpLossCount} lose). The difference comes from
            which individual contracts round above or below the 4.0
            quality-bonus eligibility line under each measure-removal model.
            <div className="export-qbp-movements mt-2 grid gap-2 md:grid-cols-2">
              <div className="export-qbp-movement rounded-lg border border-border bg-card/40 p-2">
                ...
                <p className="mt-1">
                  ...
                  <span className="font-mono text-foreground">
                    {formatContractList(qbpExplainer.model1.qbpGainers)}
                  </span>
                </p>
                <p className="mt-1">
                  <span className="text-rose-400">Loss:</span>{" "}
                  <span className="font-mono text-foreground">
                    {formatContractList(qbpExplainer.model1.qbpLosers)}
                  </span>
                </p>
              </div>
              <div className="export-qbp-movement rounded-lg border border-border bg-card/40 p-2">
                ...
                <p className="mt-1">
                  <span className="text-emerald-400">Gain:</span>{" "}
                  <span className="font-mono text-foreground">
                    {formatContractList(qbpExplainer.model2.qbpGainers)}
                  </span>
                </p>
                <p className="mt-1">
                  <span className="text-rose-400">Loss:</span>{" "}
                  <span className="font-mono text-foreground">
                    {formatContractList(qbpExplainer.model2.qbpLosers)}
                  </span>
                </p>
              </div>
            </div>
