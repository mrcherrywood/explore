  ...
  const runBacktest = useCallback(async () => {
    ...
    try {
      const res = await fetch(
        `/api/analysis/reward-factor-backtest?year=${year}`,
      );
      if (!res.ok) {
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-sm font-semibold">
              Backtest: Threshold Accuracy
            </h3>
            <p className="text-xs text-muted-foreground">
              Compute thresholds from actual CMS published measure stars and
              compare against official tech notes values
            </p>
          </div>
        </div>
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            onClick={runBacktest}
            disabled={loading}
            className="rounded-lg bg-[var(--fep-accent)] px-4 py-1.5 text-sm font-medium text-white transition hover:bg-[var(--fep-accent-deep)] disabled:opacity-50"
          >
            {loading ? (
  ...
  return (
    <div>
      <p className="mb-2 text-xs font-semibold">
        {set.label} ({set.contractCount} contracts)
      </p>
          ...
          {keys.map((k) => (
            <tr key={k} className="border-t border-border/50">
              ...
              {hasOfficial && set.differences && (
                <td
                  className={`px-2 py-1 text-right font-mono ${Math.abs(set.differences[k]) < 0.01 ? "text-emerald-500" : Math.abs(set.differences[k]) < 0.05 ? "text-amber-500" : "text-red-500"}`}
                >
                  {set.differences[k] >= 0 ? "+" : ""}
                  {fmt(set.differences[k])}
                </td>
              )}
      ...
      {!hasOfficial && (
        <p className="mt-2 text-[0.6rem] text-muted-foreground italic">
          No official thresholds available for this year. Add previous
          years&apos; tech notes data to compare.
        </p>
      )}
    </div>
  );
