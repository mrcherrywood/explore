...
export function BandMovementAnalysis() {
  const [singleData, setSingleData] = useState<BandMovementResponse | null>(
    null,
  );
  const [historicalData, setHistoricalData] =
    useState<HistoricalBandMovementResponse | null>(null);
  ...
  const fetchData = useCallback(
    async (m?: string, s?: StarRating, y?: YearSelection) => {
      const effectiveY = y ?? fromYear;
      if (
        effectiveY === "impact" ||
        effectiveY === "methodology" ||
        effectiveY === "forecast"
      ) {
        setIsLoading(false);
        return;
  ...
  const handleMeasureChange = (m: string) => {
    setMeasure(m);
    fetchData(m, star, fromYear);
  };
  const handleStarChange = (s: StarRating) => {
    setStar(s);
    fetchData(measure, s, fromYear);
  };
  const handleYearChange = (y: YearSelection) => {
    setFromYear(y);
    fetchData(measure, star, y);
  };

  const measures = singleData?.measures ?? historicalData?.measures ?? [];
  const transitions = singleData?.transitions ??
    historicalData?.transitions ?? [2023, 2024, 2025];
  const displayMeasure =
    measures.find((m) => m.normalizedName === measure)?.displayName ?? measure;

  const bucketData =
    singleData?.movement?.buckets
      .filter((b) => b.count > 0)
      .map((b) => ({
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="flex flex-wrap items-end gap-4 rounded-2xl border border-border bg-card p-4">
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-1">
            ...
            <button
              onClick={() => handleYearChange("impact")}
            ...
            <button
              onClick={() => handleYearChange("methodology")}
            ...
            <button
              onClick={() => handleYearChange("forecast")}
        ...
        singleData.movement && (
          <>
            <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
              <SummaryCard
                label="Cohort Size"
                value={String(singleData.movement.cohortSize)}
                helper={`Contracts with ${star}★ in ${fromYear} that also reported in ${(fromYear as number) + 1}${singleData.movement.avgFractionalFrom != null ? ` · avg position ${singleData.movement.avgFractionalFrom.toFixed(2)}` : ""}`}
              />
              <SummaryCard
                label="Declined"
                value={`${singleData.movement.declinedPct}%`}
                helper={
                  `${singleData.movement.declined} contracts` +
                  (singleData.movement.declinedScores.avgScoreChange !== null
                    ? ` · avg ${singleData.movement.declinedScores.avgScoreChange} pts`
                    : "")
                }
              ...
              <SummaryCard
                label="Held"
                value={`${singleData.movement.heldPct}%`}
                helper={
                  `${singleData.movement.held} contracts` +
                  (singleData.movement.heldScores.avgScoreChange !== null
                    ? ` · avg ${singleData.movement.heldScores.avgScoreChange > 0 ? "+" : ""}${singleData.movement.heldScores.avgScoreChange} pts`
                    : "")
                }
              ...
              <SummaryCard
                label="Improved"
                value={`${singleData.movement.improvedPct}%`}
                helper={
                  `${singleData.movement.improved} contracts` +
                  (singleData.movement.improvedScores.avgScoreChange !== null
                    ? ` · avg +${singleData.movement.improvedScores.avgScoreChange} pts`
                    : "")
                }
            ...
            {bucketData.length > 0 && (
              <section className="rounded-2xl border border-border bg-card p-6">
                <div className="mb-4 flex items-center gap-3">
                  <ArrowUpDown className="h-5 w-5 text-[var(--fep-accent)]" />
                  <div>
                    <h3 className="text-base font-semibold text-foreground">
                      Where did {star}
                      {"★"} contracts end up in {(fromYear as number) + 1}?
                    </h3>
                    <p className="text-xs text-muted-foreground">
                      {displayMeasure} · {singleData.movement.cohortSize}{" "}
                      contracts
                    </p>
                  </div>
                </div>
      ...
      {!isLoading &&
        !error &&
        isHistorical &&
        historicalData?.status === "ready" &&
        historicalData.history.length > 0 && (
          <BandMovementHistorical
      ...
      {!isLoading && !error && isImpact && measure && (
        <CutPointImpactAnalysis
          measure={measure}
          displayName={displayMeasure}
        />
      )}

      {/* CMS methodology backtest view */}
      {!isLoading && !error && isMethodology && measure && (
        <CutPointMethodologyAnalysis
          measure={measure}
          displayName={displayMeasure}
        />
      )}
