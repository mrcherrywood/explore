...
function normalizeChartSpec(spec: unknown): ChartSpec | null {
  ...
  const highlightKey =
    typeof spec.highlightKey === "string" ? spec.highlightKey : undefined;
  const highlightValue =
    typeof spec.highlightValue === "string" ||
    typeof spec.highlightValue === "number"
      ? spec.highlightValue
      : undefined;
  const xLabelKey =
    typeof spec.xLabelKey === "string" ? spec.xLabelKey : undefined;
  const xLabelMaxLines =
    typeof spec.xLabelMaxLines === "number" && spec.xLabelMaxLines > 0
      ? spec.xLabelMaxLines
      : undefined;
  const xLabelLineLength =
    typeof spec.xLabelLineLength === "number" && spec.xLabelLineLength > 0
      ? spec.xLabelLineLength
      : undefined;
  const xLabelAngle =
    typeof spec.xLabelAngle === "number" ? spec.xLabelAngle : undefined;
  const xLabelPadding =
    typeof spec.xLabelPadding === "number" && spec.xLabelPadding >= 0
      ? spec.xLabelPadding
      : undefined;
  const highlightLegendSelected =
    typeof spec.highlightLegendSelected === "string"
      ? spec.highlightLegendSelected
      : undefined;
  const highlightLegendPeers =
    typeof spec.highlightLegendPeers === "string"
      ? spec.highlightLegendPeers
      : undefined;
  const yAxisDomain =
    Array.isArray(spec.yAxisDomain) &&
...
export function ChartRenderer({ spec }: { spec: ChartSpec }) {
  ...
  const handleExportPNG = async () => {
    ...
    try {
      ...
      const button = chartContainerRef.current.querySelector(
        ".export-button",
      ) as HTMLElement;
      ...
      const blob = await domtoimage.toBlob(chartContainerRef.current, {
        ...
        filterStyles: (_node: unknown, propertyName: string) => {
          ...
          return !excludedProps.some((prop) =>
  ...
  if (normalized.type === "line") {
    chart = (
      <LineChart data={normalized.data}>
        ...
        <YAxis
          ...
          axisLine={false}
          tickLine={false}
          domain={normalized.yAxisDomain}
          ticks={normalized.yAxisTicks}
        />
        <Tooltip
          contentStyle={{
    ...
    const CustomTick = ({
      ...
      const lines = wrapLabel(textValue);
      const textAnchor =
        labelAngle === 0 ? "middle" : labelAngle < 0 ? "end" : "start";
    ...
    const lineCount =
      labelAngle === 0 ? Math.min(maxLines, computedLineCount) : 1;
    const angleAllowance = labelAngle === 0 ? 0 : Math.max(24, sinAngle * 80);
    const xAxisHeight = lineHeight * lineCount + angleAllowance;
    const bottomMargin =
      labelAngle === 0 ? 0 : Math.ceil(Math.max(4, angleAllowance * 0.5));

    chart = (
      <BarChart
        data={normalized.data}
        margin={{ top: 60, right: 5, left: 5, bottom: bottomMargin }}
      >
        <XAxis
          dataKey={normalized.xKey}
          stroke="#64748b"
          ...
          return (
            ...
            >
              {normalized.data.map((entry, index) => {
                ...
                const entryColor =
                  entry.__color || (highlightMatches ? "#ef4444" : baseColor);
                return (
                  <Cell
                    key={`${s.key}-${index}`}
                    fill={entryColor}
    ...
    chart = (
      <PieChart>
        ...
        <Pie
          data={normalized.data}
          dataKey={normalized.series[0]?.key}
          nameKey={normalized.xKey}
          outerRadius={110}
        >
  ...
  return (
    <div
      ...
      className="relative w-full overflow-visible rounded-3xl border border-border/30 bg-[#fffdf8] pt-5 px-5 pb-2 shadow-sm"
    >
      <button
      ...
      {normalized.title && (
        <div className="absolute bottom-2 left-3 text-[10px] text-[var(--fep-faint)] font-medium">
          {normalized.title}
        </div>
      )}
    </div>
