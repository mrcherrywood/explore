...
export function VariableSelector({ variables }: VariableSelectorProps) {
  ...
  const variablesByTable = variables.reduce(
    (acc, variable) => {
  ...
  const filteredTables = Object.entries(variablesByTable)
    .map(([table, data]) => ({
      table,
    ...
    <div className="flex flex-col gap-4">
      <div className="flex items-center justify-between gap-4">
        <input
          ...
          className="flex-1 rounded-2xl border border-white/10 bg-[#0a0a0a] px-4 py-2 text-sm text-slate-200 placeholder:text-slate-500 focus:border-[var(--fep-accent)]/40 focus:outline-none"
        />
              ...
              {tableVars.map((variable) => {
                ...
                return (
                  <button
                    key={key}
                    onClick={() =>
                      toggleVariable(variable.table, variable.column)
                    }
                  ...
                  >
                    ...
                    {isSelected && (
                      <Check className="h-3 w-3 text-[var(--fep-accent)]" />
                    )}
                  </button>
                );
              })}
      ...
      {selectedVariables.size > 0 && (
        <div className="rounded-2xl border border-[var(--fep-info-border)] bg-[var(--fep-info-bg)] p-4">
          <p className="text-xs text-[var(--fep-accent)]">
            <span className="font-semibold">
              {selectedVariables.size} variable
              {selectedVariables.size !== 1 ? "s" : ""} selected.
            </span>{" "}
            Ask the AI assistant below to analyze these variables, create
            charts, or generate insights.
          </p>
        </div>
      )}
    </div>
