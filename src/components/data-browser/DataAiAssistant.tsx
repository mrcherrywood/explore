...
export function DataAiAssistant({
  table,
  tableLabel,
  columns,
}: DataAiAssistantProps) {
  ...
  const contextPrompt = useMemo(
    () => buildContextPrompt(table, tableLabel, columns),
    [table, tableLabel, columns],
  );
  ...
  const latestSummary = useMemo(
    () => sanitizeSummary(latestAssistant?.content ?? null),
    [latestAssistant],
  );
    ...
    <div className="flex flex-col gap-6">
      <section className="rounded-3xl border border-border bg-card p-6">
        <header className="flex items-start justify-between gap-4">
          <div>
            ...
            <h2 className="mt-1 text-lg font-semibold text-foreground">
              Ask about {tableLabel}
            </h2>
            <p className="mt-2 text-xs text-muted-foreground">
              Provide filters like contract IDs, counties, or years for better
              results. Responses always reference live Supabase data.
            </p>
          </div>
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          {quickPrompts.map((prompt) => (
            <button
              ...
              className="rounded-full border border-white/10 px-3 py-1 text-xs text-slate-400 transition hover:border-[var(--fep-accent)]/40 hover:text-[var(--fep-accent)]"
            >
              {prompt}
            </button>
          ))}
        ...
        <div className="mt-5 max-h-72 space-y-4 overflow-y-auto pr-1">
          ...
          {isLoading ? (
            <div className="flex items-center gap-2 text-xs text-slate-400">
              <span className="h-2 w-2 animate-ping rounded-full bg-[var(--fep-accent)]" />
              Querying dataset…
            </div>
          ) : null}
          {error ? (
        ...
        >
          ...
          <button
            type="submit"
            className="rounded-2xl border border-[var(--fep-accent)]/50 bg-[var(--fep-band-bg)] px-4 py-2 text-sm font-medium text-[var(--fep-accent)] transition hover:border-[var(--fep-accent)] hover:text-[var(--fep-accent)] disabled:cursor-not-allowed disabled:opacity-50"
            disabled={isLoading}
          >
            Send
          </button>
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="mt-1 text-xs text-slate-500">
              Charts render automatically from the assistant&apos;s response.
            </p>
          </div>
        </div>
...
function MessageBubble({
  ...
  const components = useMemo<Components>(() => {
    return {
      ...
      table({ children }) {
        return (
          <div className="overflow-hidden rounded-xl border border-white/10">
            <table className="w-full border-collapse text-xs text-slate-200/80">
              {children}
            </table>
          </div>
        );
      ...
      thead({ children }) {
        return (
          <thead className="bg-white/5 text-[0.65rem] uppercase tracking-[0.2em] text-slate-400">
            {children}
          </thead>
        );
      },
      ...
      th({ children }) {
        return (
          <th className="px-3 py-2 text-left text-slate-200">{children}</th>
        );
      },
      td({ children }) {
    ...
    <div className={`flex gap-3 ${isUser ? "justify-end" : "justify-start"}`}>
      {!isUser ? (
        <div className="flex h-8 w-8 items-center justify-center rounded-full border border-border bg-card">
          <Bot className="h-4 w-4 text-[var(--fep-accent)]" />
        </div>
      ) : null}
      <div
      ...
      >
        <div className="mb-2 flex items-center gap-2 text-[0.6rem] uppercase tracking-[0.35em] text-slate-500">
          {isUser ? (
            ...
            <Bot className="h-3 w-3 text-[var(--fep-accent)]" />
          )}
