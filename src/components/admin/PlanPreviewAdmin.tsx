...
import { sortPlanPreviewImportFiles } from "@/lib/plan-preview/import-file-order";
import type {
...
export function PlanPreviewAdmin() {
  ...
  const importFiles = async (allFiles: File[]) => {
    ...
    const workbooks = sortPlanPreviewImportFiles(
      allFiles.filter((file) => /\.(xlsx|xls)$/i.test(file.name) && !file.name.startsWith("~$")),
    );
