...
function ThresholdPill({
  ...
  return (
    <span className="fep-pill">
      {label}: mean P65 {thresholds.mean65th.toFixed(3)} / P85{" "}
      {thresholds.mean85th.toFixed(3)} · var P30{" "}
      {thresholds.variance30th.toFixed(3)} / P70{" "}
...
function ScoreRow({ contract }: { contract: PlanPreviewFinalScore }) {
  const selected =
    contract.selectedLeg === "with_qi"
      ? contract.withQi
      : contract.selectedLeg === "without_qi"
        ? contract.withoutQi
        : null;
  ...
  return (
    <tr>
      <td
        className="l"
        style={{ maxWidth: 280, whiteSpace: "normal", lineHeight: 1.35 }}
      >
        ...
        <>
          ...
          <td>
            {selected.rewardFactor > 0 ? (
              <span className="fep-pill">
                +{selected.rewardFactor.toFixed(1)}
              </span>
            ) : (
              "0"
            )}
          ...
          <td>
            {contract.finalRating !== null ? (
              <span className="fep-pill">
                {contract.finalRating.toFixed(1)}★
              </span>
            ) : (
              "—"
            )}
...
export function PlanPreviewFinalScores({
  ...
  const [scenarioId, setScenarioId] =
    useState<PlanPreviewScenarioId>("baseline");
  const data =
    ...
    <div className="border-t" style={{ borderColor: "var(--fep-row-border)" }}>
      <div className="flex flex-wrap items-center gap-2 px-5 pb-2 pt-5">
        ...
        <select
          ...
          value={data.id}
          onChange={(event) =>
            setScenarioId(event.target.value as PlanPreviewScenarioId)
          }
        >
        ...
        {data.populationSize > 0 ? (
          <span className="fep-pill">
            {data.populationSize.toLocaleString()} contracts in population
          </span>
        ) : null}
      </div>
      <p
        className="px-5 pb-4 text-xs"
        style={{ color: "var(--fep-faint)", maxWidth: 720 }}
      >
        {data.description}
      </p>
      ...
      <div className="px-5 pb-5 pt-3">
        {data.notes.map((note) => (
          <p
            key={note}
            className="text-xs"
            style={{ color: "var(--fep-faint)", marginTop: 4 }}
          >
            {note}
          </p>
        ))}
